...
export interface TowerInstance {
  id:number; def:TowerDefinition; pos:Vec2; homePos:Vec2; hp:number; maxHp:number; status:TowerStatus;
  ...
  deployTimer:number; damagedAt:number;
}
export interface EnemyInstance {
  ...
  path:Vec2[]; groupId:number; cover:number; lane:number;
}
